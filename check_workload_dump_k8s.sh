...
echo "Starting check_workload_dump_k8s.sh..."
...
while true; do
  ...
  run_timeout "$EXEC_TIMEOUT_SEC" "$ps_out" "$ps_rc" kubectl get pods -n "$NAMESPACE" --no-headers -o custom-columns=":metadata.name"
  rc=$(cat "$ps_rc" 2>/dev/null || echo 1)
  
  if [[ "$rc" != "0" ]]; then
    rm -f "$ps_out" "$ps_rc"
    echo "warning: kubectl get pods failed/timeout rc=${rc}"
    sleep "$INTERVAL_SEC"
    continue
  fi
  rm -f "$ps_rc"
  ...
  echo "found ${#pods[@]} pods matching '${NAME_PREFIX}'"

  if (( ${#pods[@]} == 0 )); then
    echo "no running pods matching ^${NAME_PREFIX}"
    sleep "$INTERVAL_SEC"
    continue
  fi

  for p in "${pods[@]}"; do
    echo "[$p] dumping processcache..."

    phase=$(kubectl get pod -n "$NAMESPACE" "$p" -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
    if [[ "$phase" != "Running" ]]; then
      echo "[$p] skipped (phase=$phase)"
      continue
    fi
    ...
    run_timeout "$EXEC_TIMEOUT_SEC" "$tmp_cache" "$dump_rc" kubectl exec -n "$NAMESPACE" -c "$CONTAINER_NAME" "$p" -- bash -lc "tetra dump processcache --exclude-execve-map-processes --skip-zero-refcnt --max-recv-size ${MAX_RECV_SIZE}"
    rc=$(cat "$dump_rc" 2>/dev/null || echo 1)
    
    if [[ "$rc" != "0" ]]; then
      rm -f "$tmp_cache" "$dump_rc"
      echo "[$p] warning: failed to dump processcache (rc=$rc)"
      continue
    fi
    rm -f "$dump_rc"
    ...
    found_dump="${HOST_OUT_DIR}/CHECK_${ts}_${p}_${record_count}.json"
    ...
    if ! jq -r 'select(.refcnt > 0) | .refcnt_ops | keys | join(" ")' "$found_dump" > "$ops_list"; then
        echo "[$p] error: jq failed to parse dump"
        rm -f "$ops_list"
        continue
    fi
